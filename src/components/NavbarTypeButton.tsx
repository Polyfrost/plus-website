...
export default function NavbarTypeButton({ atTop, extended, image, name, nav }: { atTop: boolean; extended: boolean; image: string; name: string; nav: string }) {
    ...
    return (
        <button onClick={() => router.push(nav)} className="group flex flex-col gap-0.5 shrink-0 hover:scale-105 duration-300">
            ...
            />
            <p className={`text-white/75 light:text-black/75 group-hover:text-white group-hover:light:text-black transition-all duration-300 ease-out ${atTop || extended ? "text-lg" : "text-sm"} leading-6 text-center`}>{name}</p>
        </button>
    );
}
